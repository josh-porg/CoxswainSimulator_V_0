...
print(f"\nslide travel     {hip[:,0].ptp():.3f} m   target 0.60-0.70")
print(f"seat height var  {hip[:,2].ptp()*1000:.2f} mm  target ~0 (level track)")
print(f"handle x travel  {hand[:,0].ptp():.3f} m")
print(f"handle z rise    {hand[:,2].ptp():.3f} m")
print(f"crew CoM x range {com[:,0].ptp():.3f} m   target 0.45-0.60")
print(f"crew CoM z range {com[:,2].ptp():.3f} m")
